  ...
  <div class="content">
    <div class="popUpMask" v-if="if_buy" v-bind:class="{ active_card: if_buy }">
      ...
      >
        ...
        <div>
          <div>You succesfully added to your cart</div>
    ...
    <div class="itemPageHeaer">
      ...
      <div class="itemInfo">
        ...
        <div class="reviews">
          <span
            style="margin-left: 0"
            v-for="sub_item in product.stars"
            :key="sub_item"
          >
            <img
              v-if="sub_item == 's'"
          ...
          <div class="buyItemButton">
            ...
            <div>
              ...
              <div
                ...
                @click="set_sku_favor()"
              >
                <img src="/imgGameLogoPurple/Favorite.svg" alt />
              </div>
              <div
...
export default {
  head() {
    return {
      ...
      meta: [
        {
          hid: this.product.seo_descr,
          name: this.product.seo_descr,
  ...
  async asyncData({ $axios, route, store, redirect,res }) {
      ...
      let rev_result = 0;
        ...
        let temp = route.params.sku;

        product = JSON.parse(temp);
        chars = product.chars;
      
      /*let response_coin_settings = await $axios
        .$get(apiUrl + "coins")
        .then((data) => {
      ...
      if (product.raid_enable == 1) {
        let response_data_event_rel = await $axios
          .$get(
            apiUrl +
              "events?filter=date,eq," +
          ...
          .then((data) => {
            ...
            if (events_by_product) {
              ...
              let response_data_times_cest_rel = $axios
                .$get(
                  apiUrl +
                    "times?filter=parentid,eq," +
              ...
              let response_data_times_edt_rel = $axios
                .$get(
                  apiUrl +
                    "times?filter=parentid,eq," +
                    events_by_product.id +
  ...
  mounted() {
    this.getCookie_zone(),
      ...
      this.init_step(),
      console.log(this.rank_option),
      console.log(this.rank_selects);
  },
  methods: {
    init_step() {
    ...
    set_sku_favor() {
      ...
      this.check_favorite();
      try {
        dashly.track("Пользователь добавил товар в избранное", {"product": this.product.menu_name})
      } catch (e) {
    ...
    delete_sku_favor() {
      ...
      this.check_favorite();
      try {
        dashly.track("Пользователь удалил товар из избранного", {"product": this.product.menu_name})
      } catch (e) {
        console.log("dashly not loaded")
      }
      location.reload();
